...
function App() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setAuthNotice(error instanceof Error ? error.message : 'Google ile giriş başlatılamadı.')
    }
  }
          ...
          <section className="panel login-panel">
            ...
            <div className="panel-body">
              ...
              <div style={{ display: 'flex', alignItems: 'center', margin: '16px 0', color: 'var(--text-muted)' }}>
                ...
                <span style={{ padding: '0 12px', fontSize: '0.85rem' }}>veya</span>
                <hr style={{ flex: 1, border: 'none', borderTop: '1px solid var(--border)' }} />
              </div>
              ...
              {authNotice ? <p className="inline-note" style={{ marginTop: '16px' }}>{authNotice}</p> : null}
            </div>
          </section>
        ) : (
          <>
